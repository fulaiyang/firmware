...
bda_init(void)
{
    ...
    if (!CONFIG_MALLOC_UPPERMEMORY)
        ebda_seg = FLATPTR_TO_SEG(ALIGN_DOWN(SYMBOL(final_varlow_start), 2048)
...
maininit(void)
{
    ...
    // before printf the logo, it should make some rom region read only,
    // or it will touch the rom and lead to screen crash
