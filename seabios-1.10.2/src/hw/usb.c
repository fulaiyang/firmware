...
get_device_config(struct usb_pipe *pipe)
{
    ...
    struct usb_config_descriptor *cfg_tmp;
    ...
    cfg_tmp = (struct usb_config_descriptor *)config;
    if (cfg_tmp->bDescriptorType != USB_DT_CONFIG || cfg.wTotalLength != cfg_tmp->wTotalLength)
    {
        dprintf(1, "Get the config descriptor abnormal\n");
        free(config);
        return NULL;
    }
...
struct usb_common_header {
    u8 bLen;
    u8 bType;
};

#define US_PR_KEYBOARD         0x1
#define US_PR_MOUSE            0x2
/*
 * return first interface, or keyboard interface with multi interface devices
*/
static struct usb_interface_descriptor * find_interface(const void * config_in, size_t len)
{
    int i = 0;
    struct usb_interface_descriptor *iface;
    struct usb_interface_descriptor *ret_iface = NULL;
    struct usb_common_header *hdr = (struct usb_common_header *)config_in;;
    while(i < len) {
        if (hdr->bType == USB_DT_INTERFACE) {
            iface = (struct usb_interface_descriptor *) hdr;
            if(ret_iface == NULL)
                ret_iface = iface;
            if ((iface->bInterfaceClass == USB_CLASS_HID) && (iface->bInterfaceProtocol == US_PR_KEYBOARD)) {
                ret_iface = iface;
                dprintf(3, "found keyboard hid\n");
                break;
            }
        }
        if (hdr->bLen <= 0) {
        ...
        i += hdr->bLen;
        hdr = (struct usb_common_header *)((u8 *)config_in + i);
    }
    return ret_iface;
}
...
configure_usb_device(struct usbdevice_s *usbdev)
{
    ...
    dprintf(1, "config:bLength[%d], bDescriptorType[%d], wTotalLength[%d], bNumInterfaces[%d], bConfigurationValue[%d], iConfiguration[%d], bmAttributes[%d], bMaxPower[%d]\n", config->bLength, config->bDescriptorType, config->wTotalLength, config->bNumInterfaces, config->bConfigurationValue, config->iConfiguration, config->bmAttributes, config->bMaxPower);
    
    // Determine if a driver exists for this device - only look at the
    // first interface of the first configuration.
    struct usb_interface_descriptor *iface = find_interface(config, config->wTotalLength);
