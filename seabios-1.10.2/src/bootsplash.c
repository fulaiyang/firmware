...
enable_bootsplash(void)
{
    ...
    if (type == 0) {
        ...
        /* the bmp bpp property is got from the bmp source */
        bmp_get_size(bmp, &width, &height, &bpp_require);
    }
    ...
    /* we don't need bmp convert,it is convert by linux. It can save time */
    ...
    /* printf from the source file,behind 54 bytes is the picture data */
    iomemcpy(framebuffer, filedata + 54, imagesize);
