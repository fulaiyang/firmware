...
void intel_igd_setup(struct pci_device *dev, void *arg)
{
    ...
    u64 igd_fit_host = le64_to_cpu(romfile_loadint("etc/igd-fit-host", 0));
    u64 bdsm_base;
    void *addr;
    u16 bdf = dev->bdf;
    ...
    if (bdsm_size && (bdf == pci_to_bdf(0, 2, 0))) {
        ...
        if (igd_fit_host == 0) {
            ...
            bdsm_base = le64_to_cpu(romfile_loadint("etc/igd-bdsm-base", 0));

            e820_add((u32)bdsm_base, bdsm_size, E820_RESERVED);

            dprintf(1, "QEMU assign IGD BDSM at 0x%08x, size %lldMB, dev 00:02.0,"
                       "the same as host configuration \n",
                             (u32)bdsm_base, bdsm_size >> 20);
        }
    }
}
 ...
 * before i440fx_mem_addr_setup pcimemstart=ramsize
 * RamSizeOver4G=0 has a new hotplug mem below max_ram_below_4g
 ****************************************************************/

static void i440fx_mem_addr_setup(struct pci_device *dev, void *arg)
{
    if (RamSizeOver4G == 0) {
        u64 max_ram_below_4g = romfile_loadint("etc/max-ram-below-4g", 0);
        if (max_ram_below_4g != 0)
            pcimem_start = max_ram_below_4g;
    }

    if (pcimem_start <= 0x80000000)
        pcimem_start = 0x80000000;
    else if (pcimem_start <= 0xc0000000)
